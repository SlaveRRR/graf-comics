...
const SortFilter: FC<Props> = ({ sortText, setSort, setActive, isActive, filters, header }) => {
  return (
    <div className={styles['sort-filter__container']}>
      <button onClick={() => setActive()} className={styles['sort-filter__header']}>
        {header}{' '}
        <svg
          className={cn(
            {
            ...
            styles['sort-filter__arrow-icon'],
          )}
          width="8"
          height="7"
          viewBox="0 0 8 7"
      ...
      <div className={styles['sort-filter__content']}>
        {isActive &&
          filters.map((text, i) => (
            <label htmlFor={`${text}id`} className={cn(styles['sort-filter__sort'])}>
              <span className={styles['sort-filter__radio-btn']}></span>
              <input
                checked={sortText === text}
                onClick={() => setSort(text)}
                name="radiobtn"
                id={`${text}id`}
                type="radio"
                className={styles['sort-filter__sort-inp']}
              />{' '}
              {text}
            </label>
          ))}
