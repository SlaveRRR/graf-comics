...
import { BackSvg } from 'public/back';
      ...
      {active ? (
        <form onSubmit={handleSubmit(handleVerifyEmail)} className={styles['registration__form']}>
          ...
          <input
            {...register('email', {
              required: true,
              pattern: { value: emailRegexp, message: 'Введите правильный email' },
            })}
  ...
  return (
    <div className={styles['box']}>
      <form onSubmit={handleSubmit(signUpHandler)} className={styles['registration-box']}>
        <a className={styles['container__backLink']} href="http://localhost:3000/">
          <BackSvg />
        ...
        <div className="container">
          ...
          <p className={styles['registration-box__with-social']}>через соцсети</p>
          <SocialAuthLinks mixClass={[styles['registration-box__social']]} />
          <p className={styles['registration-box__or']}>или</p>
          ...
          {Boolean(errors?.password) && (
            <p className={styles['registration-box__error']}>{errors?.password?.message}</p>
          )}
